...
export default function OnboardingForm({
  ...
  const [facultyId, setFacultyId] = useState<string | null>(initialFacultyId);
    ...
    fetch("/api/faculties")
      ...
      .then((data) => {
        ...
        setFacultyId((current) => {
          if (current && data.faculties.some((f: Faculty) => f.id === current)) return current;
          return data.faculties[0]?.id ?? null;
        });
      });
  }, []);
  ...
  useEffect(() => {
    setGroupId((current) => {
      if (current && selectedFaculty?.groups.some((g) => g.id === current)) return current;
      return selectedFaculty?.groups[0]?.id ?? null;
    });
  }, [selectedFaculty]);
  ...
  async function handleSubmit() {
    ...
    router.push(role === "TEACHER" ? "/journal" : redirectTo);
    router.refresh();
  }
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-bg pt-[var(--tg-safe-top)]">
      <div className="flex-shrink-0 px-4 pb-2.5 pt-3.5">
        <h1 className="text-xl font-bold tracking-tight text-text">{heading}</h1>
      ...
      <div className="flex-shrink-0 border-t border-border px-4 pt-3 pb-[calc(24px+var(--tg-safe-bottom))]">
        <button
          type="button"
          disabled={!canSubmit || submitting}
          onClick={handleSubmit}
          className="w-full rounded-control bg-primary py-3.5 text-center text-base font-semibold text-white disabled:opacity-50 [touch-action:manipulation]"
        >
